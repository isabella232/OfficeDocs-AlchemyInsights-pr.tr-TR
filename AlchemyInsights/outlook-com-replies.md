---
title: 9000240 Outlook.com yanıtla
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom:
- "1825"
- "9000240"
ms.openlocfilehash: 0fece3194ab9a14b8fb21123b0b92c2a9aabba62
ms.sourcegitcommit: 631cbb5f03e5371f0995e976536d24e9d13746c3
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 04/22/2020
ms.locfileid: "43743797"
---
# <a name="replying-in-outlookcom"></a>Outlook.com yanıtlama

Aynı anda bir iletiyi yanıtlamak için:

1. İleti listenizde, yanıtlamak istediğiniz iletiyi seçin.
2. İleti bölmesinin sağ üst kısmında, <img src='data:image/png;base64,' /> **Yanıtla** veya <img src='data:image/png;base64,' /> **Tümlerini yanıtla**.
3. İletinizi yazın ve **gönder'i**seçin.

Tüm iletileri otomatik olarak yanıtlamak için:

1. Git <img src='data:image/png;base64,' />
**Ayarlar** > Otomatik yanıt ayarlarınızı açmak için**tüm Outlook Ayarlarını** > **Posta** > **Otomatik yanıtları** görüntüleyin. [Automatic replies settings](https://outlook.live.com/mail/options/mail/automaticReplies)
2. **Otomatik yanıtları aç** geçiş düğmesini seçin.
3. **Yanıtgönder'in yalnızca bir zaman dilimi** onay kutusunda seçilmesi şunları yapmanızı sağlar:
    - Yalnızca seçtiğinizde yanıt gönderin. Bir süre ayarlamazsanız, siz kapatana kadar otomatik yanıtınız geçerli kalır.
    - Takviminizi engelleme
    - Yeni davetleri otomatik olarak reddetme
    - Bu süre boyunca toplantılarımı reddet ve iptal et
4. İleti kutusuna, uzakta olduğunuz süre boyunca insanlara gönderilecek bir ileti yazın.
5. Yalnızca kişilerinize yanıt göndermek için, yalnızca kişileri onay **kutusuna yanıt gönder'i** seçin.
6. **Kaydet**'i seçin.

[Outlook.com'da otomatik yönlendirme](https://support.office.com/article/14614626-9855-48dc-a986-dec81d07b1a0?wt.mc_id=Office_Outlook_com_Alchemy)hakkında daha fazla bilgi edinin.