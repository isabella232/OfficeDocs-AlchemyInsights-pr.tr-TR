---
title: '8000019: Kişileri ekleme veya içeri aktarma'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 8e9535da046f21cc35ac9704492d8cd04e770aa997a6995f00dd3953241e471b
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 08/05/2021
ms.locfileid: "53999800"
---
# <a name="import-contacts"></a>Kişileri içeri aktarma

Kişileri e-posta Web üzerinde Outlook aktarmadan önce, bunları diğer e-posta sağlayıcınızdan bir dosya olarak dışarı .csv gerekir. Ardından aşağıdaki yönergeleri izleyin.

1. Sayfanın alt kısmında, <img src='data:image/png;base64,' />
 **Kişiler**.
2. Araç çubuğunda Kişileri içeri  >  **aktar'ı seçin.**
3. **Gözat'ı** seçin, CSV dosyanızı seçin ve sonra da Aç'ı **seçin.**
4. İçeri **Aktar'ı seçin.**

Daha fazla bilgi edinmek için [bkz. Kişileri başka bir Outlook.](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web)

