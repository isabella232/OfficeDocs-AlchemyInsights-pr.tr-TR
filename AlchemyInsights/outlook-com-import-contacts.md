---
title: Outlook.com'daki kişileri içe aktarma
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "278"
- "8000019"
ms.assetid: 921c5c7c-9a02-4682-9bd1-03ef9bb0fd6e
ms.openlocfilehash: 6c115469733c0be6a8d18a700cba7e4b3088937b
ms.sourcegitcommit: 631cbb5f03e5371f0995e976536d24e9d13746c3
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 04/22/2020
ms.locfileid: "43760180"
---
# <a name="import-contacts"></a>İlgili kişileri içe aktarma

Kişileri Outlook.com'a aktarmadan önce, bunları diğer e-posta sağlayıcınızdan .csv dosyası olarak dışa aktarmanız gerekir. Ardından, aşağıdaki talimatları izleyin.
  
1. [İnsanlara](https://outlook.live.com/people/) Git <img src='data:image/png;base64,' />
.
2. Sayfanın üst **kısmında, Alma bağlantılarını** **yönet'i** \> seçin.
3. **Gözat'ı**seçin, dışa aktarılan kişilerinizi kaydettiğiniz klasöre gidin ve **Aç'ı**seçin.

Daha fazla bilgi için [Outlook.com için Kişileri Aktar'a](https://support.office.com/article/285a3b55-8d93-4ac8-93df-43fffd13b2f1?wt.mc_id=Office_Outlook_com_Alchemy)bakın.